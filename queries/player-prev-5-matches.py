...
def get_next_fixtures(player_id):
    return con.execute(f"""
        ...
        JOIN teams t ON (CASE WHEN f.team_h = p.team THEN f.team_a ELSE f.team_h END) = t.id
...
def main():
    ...
    while True:
        ...
        next_fixtures = get_next_fixtures(player_id)
        if not next_fixtures:
            print("No upcoming fixtures found for this player.")
            continue

        player_name = con.execute(f"SELECT full_name FROM players WHERE id = {player_id}").fetchone()[0]
        print(f"\nNext 5 fixtures for {player_name}:")
        print(tabulate(next_fixtures, headers=["Opponent Team ID", "Gameweek", "Venue", "Opponent Name"], tablefmt="grid"))

        for fixture in next_fixtures:
            opponent_team, gameweek, venue, opponent_name = fixture
            print(f"\nAnalyzing fixture: Gameweek {gameweek}, {venue} against {opponent_name}")

            historical_results, _, _ = get_historical_results(player_id, opponent_team)

            print(f"Historical results against {opponent_name}:")
